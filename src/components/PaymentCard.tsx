...
const PaymentCard: React.FC<PaymentCardProps> = ({ payment, account }) => {
  return (
    <Card className="w-full max-w-sm  bg-zinc-900 text-white border-gray-700">
      <CardHeader>
        <CardTitle className="text-lg flex flex-row gap-2 font-semibold">
          {payment.sender == account ? "Payment Sent" : "Payment Received"}
          {payment.sender == account ? (
            <CircleCheckBig color="green" strokeWidth={2} />
          ) : (
            <HandCoins color="green" strokeWidth={2} />
          )}
        </CardTitle>
      ...
      <CardContent>
        <div className="flex justify-between items-center mb-2">
          ...
          <Badge
            variant="secondary"
            className={`${payment.sender == account ? "text-red-400 bg-red-900" : "text-green-400 bg-green-900"}`}
          >
      ...
      <CardFooter className="text-xs font-medium text-gray-400">{payment.timestamp}</CardFooter>
    </Card>
  );
};
